...
| 项 | fork 位置 | 决策 | 状态 | 注意点 |
|---|---|---|---|---|
| composer 集群 `:has(textarea)` 正向加 `[data-composer-input]` | `7f47f9e`，`layout.css.ts` / `misc.css.ts` | 合并 | 待摘 | 与本仓库 PR #47 的 `:not(:has([data-composer-input]))` 排除型用法方向相反，两套对账合并后需重跑 CDP 探针（composer 三件套 + 窄屏压力）。 |
